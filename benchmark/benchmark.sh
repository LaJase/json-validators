...
echo -e "  ${GREEN}✓ ${RESET}$(file "$RUST_BIN" | grep -oP 'ELF.*' | head -c60)"

sep
echo -e "${CYAN}► Building Go...${RESET}"
(cd "$ROOT/go" && go mod tidy -e 2>/dev/null; go build -o validator .)
...
PF_LABELS+=("Rust  "); PF_TIMES+=("$ELAPSED"); PF_OK+=("$OK"); PF_FAIL+=("$FAIL")
...
echo -e "${GREEN}done${RESET}"
BT_LABELS+=("Rust  "); BT_TIMES+=("$ELAPSED")
